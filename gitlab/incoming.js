...
See: ${data.object_attributes.url}`
  ...
  pipelineEvent(data) {
		...
		const pipeid = data.object_attributes.id;
		const attachments = [{
			author_name: data.user.name,
			author_icon: data.user.avatar_url,
			title: `Pipeline ${pipeid} (${data.object_attributes.duration}s) - ${data.object_attributes.status}`,
			title_link: data.project.web_url,
			color: NOTIF_COLOR
		}];

		if (Array.isArray(data.builds)) {
			for (const build of data.builds) {
				if (build.when === "manual") {
					attachments.push({
						author_name: `Build - #${build.id}`,
						title: `${build.name}`,
						title_link: `${data.project.web_url}/builds/${build.id}`,
						text: `status: ${build.status}\nstage: ${build.stage}`,
						color: BUILD_COLORS[build.status]
					});
				}
			}
		}
